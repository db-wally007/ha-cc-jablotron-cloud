...
## Architecture
- `JablotronClient` (jablotron.py) - holds ONE persistent `Jablotron` bridge; `_api_call()` wraps every API call: logs in only when there is no session, and on `UnauthorizedException`/`SessionExpiredException` (Jablotron sessions expire ~30min server-side) re-logins once and retries — purely reactive, no time-based refresh; auth errors only reach callers when the re-login itself fails (genuinely bad credentials). Typed wrapper methods (`get_sections`, `control_section`, ...) are the only public API surface.
- `JablotronDataCoordinator` (__init__.py) - polls API, mutates `client.services` dict in-place; `coordinator.data` is unused (stays None). Per-service `JablotronServiceCapabilities` recorded at setup gate polling: endpoints that returned no data (no gates/thermo/sections) are never polled again until reload. On 429, `update_interval` is stretched to `Retry-After` so no poll fires during the backoff; it is restored after the next successful update.
