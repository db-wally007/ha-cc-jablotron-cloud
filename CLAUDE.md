...
## Architecture
- `JablotronClient` (jablotron.py) - holds ONE persistent `Jablotron` bridge; `_api_call()` wraps every API call: ensures login (proactive refresh after `SESSION_MAX_AGE`, since Jablotron sessions expire ~30min), and on `UnauthorizedException`/`SessionExpiredException` re-logins once and retries — auth errors only reach callers when the re-login itself fails (genuinely bad credentials). Typed wrapper methods (`get_sections`, `control_section`, ...) are the only public API surface.
- `TooManyRequestsException` (jablotron.py) - fallback stub for planned jablotronpy 429/`Retry-After` support; remove once upstream ships it and the manifest pin is bumped
- `JablotronDataCoordinator` (__init__.py) - polls API, mutates `client.services` dict in-place; `coordinator.data` is unused (stays None). Per-service `JablotronServiceCapabilities` recorded at setup gate polling: endpoints that returned no data (no gates/thermo/sections) are never polled again until reload. On 429, `_backoff_until` suppresses all API calls until `Retry-After` elapses.
